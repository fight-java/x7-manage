...
var KisBpmShapeSelectionCallactivityTypeCtrl = [ '$rootScope', '$scope', '$timeout', '$translate','$http', function($rootScope, $scope, $timeout, $translate,$http) {
    ...
    var initZtree = function(){
    	var ztreeCreator = new ZtreeCreator('type__Tree_',window.parent.getContext().portal + "/sys/sysType/v1/getTypesByKey?typeKey=FLOW_TYPE")
    	.setCallback({onClick:selectType})
    	.initZtree({reqMethod:"get"},function(treeObj){
    		_treeObj = treeObj;
    	}); 
    	new ZtreeCreator('type__Tree_',window.parent.getContext().portal + "/sys/sysType/v1/getTypesByKey")
    	.setCallback({onClick:selectType})
    	.initZtree({typeKey:"FLOW_TYPE"},function(treeObj){}); 
    };
    
    var selectType = function(event, treeId, treeNode){
    	$http({method: 'GET',headers: { 'Authorization':KISBPM.URL.getToken}, url: window.parent.getContext().bpmModel + "/flow/def/v1/getByTypeId?typeId="+treeNode.id}).
